/**
 * 角色管理 Mutation Hook
 * 直接使用 React Query 的 useMutation，提供角色管理的增删改操作
 */

import { useCallback, useMemo } from 'react';
import { useMutation, useQueryClient } from '@tanstack/react-query';
import { message } from 'antd';
import { rolesApi, type Role, type CreateRoleDto, type UpdateRoleDto } from '@/lib/api/roles';
import type { PaginatedResponse } from '@/lib/api/types';

/**
 * useRolesMutation 配置
 */
export interface UseRolesMutationOptions {
  /**
   * 成功提示文本
   */
  successMessages?: {
   ...
   */
  onSuccess?: {
    create?: (data: Role) => void | Promise<void>;
    update?: (data: Role) => void | Promise<void>;
...
/**
 * useRolesMutation 返回值
 */
export interface UseRolesMutationReturn {
  /**
   * 创建 Mutation
   */
  createMutation: ReturnType<typeof useMutation<Role, Error, CreateRoleDto>>;
  /**
   * 更新 Mutation
   */
  updateMutation: ReturnType<typeof useMutation<Role, Error, { id: string; data: UpdateRoleDto }>>;
  /**
   * 删除 Mutation
   */
 ...
 * 角色管理 Mutation Hook
 */
export function useRolesMutation(options: UseRolesMutationOptions = {}): UseRolesMutationReturn {
  const {
    successMessages = {},
    errorMessages = {},
  ...
  const createMutation = useMutation({
    ...
    onMutate: async (newRole) => {
      ...
      const previousData = queryClient.getQueryData<PaginatedResponse<Role>>(queryKey);
      if (previousData) {
        queryClient.setQueryData<PaginatedResponse<Role>>(queryKey, {
          ...previousData,
          items: [{ ...newRole, id: 'temp-' + Date.now() } as Role, ...previousData.items],
  ...
  const batchDeleteMutation = useMutation({
    mutationFn: async (ids: (string | number)[]) => {
      await Promise.all(ids.map((id) => rolesApi.deleteRole(String(id))));
    },
    onMutate: async (ids) => {
      await queryClient.cancelQueries({ queryKey });
      const previousData = queryClient.getQueryData<PaginatedResponse<Role>>(queryKey);
  ...
  const create = useCallback(
    async (data: CreateRoleDto): Promise<Role> => {
  ...
  const update = useCallback(
    async (id: string, data: UpdateRoleDto): Promise<Role> => {
