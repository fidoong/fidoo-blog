/**
 * 菜单管理 Mutation Hook
 * 直接使用 React Query 的 useMutation，提供菜单管理的增删改操作
 */
 ...
 * useMenusMutation 配置
 */
export interface UseMenusMutationOptions {
  /**
   * 成功提示文本
   */
  successMessages?: {
   ...
   */
  onSuccess?: {
    create?: (data: Menu) => void | Promise<void>;
    update?: (data: Menu) => void | Promise<void>;
...
/**
 * useMenusMutation 返回值
 */
export interface UseMenusMutationReturn {
  /**
   * 创建 Mutation
   */
  createMutation: ReturnType<typeof useMutation<Menu, Error, CreateMenuDto>>;
  /**
   * 更新 Mutation
   */
  updateMutation: ReturnType<typeof useMutation<Menu, Error, { id: string; data: UpdateMenuDto }>>;
  /**
   * 删除 Mutation
   */
   ...
   * 更新
   */
  update: (id: string, data: UpdateMenuDto) => Promise<Menu>;
  /**
   * 删除
   */
 ...
 * 菜单管理 Mutation Hook
 */
export function useMenusMutation(options: UseMenusMutationOptions = {}): UseMenusMutationReturn {
  const {
    successMessages = {},
    errorMessages = {},
